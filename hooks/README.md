...
**Deployed hook artifacts** — the actual files installed on user machines by `stc init`. These are shell scripts, TypeScript plugins, and rules files that run outside the Rust binary. They are **thin delegates**: parse agent-specific JSON, call `stc rewrite` as a subprocess, format agent-specific response. Zero filtering logic lives here.
...
LLM agent integrations that intercept CLI commands and route them through RTK for token optimization. Each hook transparently rewrites raw commands (e.g., `git status`) to their RTK equivalents (e.g., `stc git status`), delivering 60-90% token savings without requiring the agent or user to change their workflow.
...
Agent runs command (e.g., "cargo test --nocapture")
  ...
  -> Calls `stc rewrite "cargo test --nocapture"`
...
All rewrite logic lives in the Rust binary (`src/discover/registry.rs`). Hook scripts are **thin delegates** that handle agent-specific JSON formats and call `stc rewrite` for the actual decision. This ensures a single source of truth for all 70+ rewrite patterns.
...
| VS Code Copilot Chat | Rust binary (`stc hook copilot`) | Transparent rewrite | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary (`stc hook copilot`) | Deny-with-suggestion | No (agent retries) |
| Cursor | Shell hook (`preToolUse`) | Transparent rewrite | Yes (`updated_input`) |
| Gemini CLI | Rust binary (`stc hook gemini`) | Transparent rewrite | Yes (`hookSpecificOutput`) |
...
```json
{
  ...
  "permissionDecisionReason": "Token savings: use `stc git status` instead"
}
```
...
```typescript
const result = await $`stc rewrite ${command}`.quiet().nothrow()
...
Example: `cargo fmt --all && cargo test` becomes `stc cargo fmt --all && stc cargo test`
...
- **Already-RTK**: `stc git status` passes through unchanged (no `stc stc git`)
...
- stc binary not found: warning to stderr, exit 0
- stc version too old (< 0.23.0): warning to stderr, exit 0
- Invalid JSON input: pass through unchanged
- `stc rewrite` crashes: hook exits 0 (subprocess error ignored)
