...
- **Transparent rewrite via a programmatic `PreToolUse` hook.** `stc init --codex`
  registers `stc hook codex` in Codex's `hooks.json` with `matcher = "Bash"`.
...
- Codex 0.133.0 only applies `updatedInput` when paired with
  ...
  explicit allow — mirroring the Cursor hook. Unsupported commands, stc deny-rule
...
- Installed to `$CODEX_HOME` when set, otherwise `~/.codex/`, by `stc init --codex`
  (project mode writes `.codex/hooks.json`). `hooks.json` is merged idempotently and
  the entry is removed cleanly by `stc init -g --codex --uninstall`.
