...
lazy_static! {
    ...
    // `==> name <==` banners that `stc read --max-lines` cannot reproduce.
...
pub fn rewrite_command(
    ...
    // Simple (non-compound) already-stc command — return as-is.
    // For compound commands that start with "stc" (e.g. "stc git add . && cargo test"),
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("stc ") || trimmed == "stc") {
...
fn rewrite_line_range(cmd: &str) -> Option<String> {
    for re in [&*HEAD_N, &*HEAD_LINES] {
        if let Some(caps) = re.captures(cmd) {
            ...
            return Some(format!("stc read {} --max-lines {}", file, n));
        }
    }
    ...
    for re in [
        ...
        if let Some(caps) = re.captures(cmd) {
            let n = caps.get(1)?.as_str();
            let file = caps.get(2)?.as_str();
            return Some(format!("stc read {} --tail-lines {}", file, n));
        }
    }
    None
}
...
fn rewrite_segment_inner(
    ...
    // Already stc — pass through unchanged
    if cmd_part.starts_with("stc ") || cmd_part == "stc" {
        return Some(trimmed.to_string());
    }
    ...
    // semantics than stc read or no equivalent at all. Only `-n` (line numbers)
    // maps correctly to `stc read -n`. Skip rewrite for any other flag.
    ...
    if let Some(parts) = parse_golangci_run_parts(cmd_part) {
        let rewritten = if parts.global_segment.is_empty() {
            format!("stc golangci-lint {}", parts.run_segment)
        } else {
            format!(
                "stc golangci-lint {} {}",
    ...
    // stc gh would corrupt — skip rewrite so the caller gets raw JSON.
    if rule.rtk_cmd == "stc gh" {
...
mod tests {
    ...
    fn test_classify_rtk_already() {
        assert_eq!(classify_command("stc git status"), Classification::Ignored);
    }
    ...
    fn test_rewrite_git_dash_c_status() {
        assert_eq!(
            ...
            Some("stc git -C /path/to/repo status".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_log() {
        assert_eq!(
            ...
            Some("stc git -C /tmp/myrepo log --oneline -5".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_diff() {
        assert_eq!(
            ...
            Some("stc git -C /home/user/project diff --name-only".into())
        );
    }
    ...
    fn test_rewrite_compound_and() {
        assert_eq!(
            ...
            Some("stc git add . && stc cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_three_segments() {
        assert_eq!(
            ...
            Some("stc cargo fmt --all && stc cargo clippy --all-targets && stc cargo test".into())
        );
    }
    ...
    fn test_rewrite_already_rtk() {
        assert_eq!(
            rewrite_command_no_prefixes("stc git status", &[]),
            Some("stc git status".into())
        );
    }
    ...
    fn test_rewrite_background_single_amp() {
        assert_eq!(
            ...
            Some("stc cargo test & stc git status".into())
        );
    }
    ...
    fn test_rewrite_with_env_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh stc git push".into())
        );
    }
    ...
    fn test_rewrite_cat_file() {
        assert_eq!(
            ...
            Some("stc read src/main.rs".into())
        );
    }

    #[test]
    fn test_rewrite_cat_with_incompatible_flags_skipped() {
        // cat flags with different semantics than stc read — skip rewrite
    ...
    fn test_rewrite_cat_with_compatible_flags() {
        // cat -n (line numbers) maps to stc read -n — allow rewrite
        assert_eq!(
            rewrite_command_no_prefixes("cat -n file.txt", &[]),
            Some("stc read -n file.txt".into())
        );
    }
    ...
    fn test_rewrite_pipe_first_only() {
        ...
        assert_eq!(
            ...
            Some("stc git log -10 | grep feat".into())
        );
    }

    #[test]
    fn test_rewrite_find_pipe_skipped() {
        // find in a pipe should NOT be rewritten — stc find output format
    ...
    fn test_rewrite_mixed_compound_partial() {
        ...
        assert_eq!(
            rewrite_command_no_prefixes("stc git add . && cargo test", &[]),
            Some("stc git add . && stc cargo test".into())
        );
    }
    ...
    fn test_rewrite_non_rtk_disabled_env_still_rewrites() {
        assert_eq!(
            ...
            Some("SOME_VAR=1 stc git status".into())
        );
    }
    ...
    fn test_rewrite_env_quoted_value_with_spaces() {
        assert_eq!(
            ...
            Some(r#"GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" stc git push"#.into())
        );
    }
    ...
    fn test_rewrite_env_single_quoted_value_with_spaces() {
        assert_eq!(
            ...
            Some("EDITOR='vim -u NONE' stc git commit".into())
        );
    }
    ...
    fn test_rewrite_env_quoted_plus_unquoted() {
        assert_eq!(
            ...
            Some(r#"FOO="bar baz" BAR=1 stc git status"#.into())
        );
    }
    ...
    fn test_rewrite_env_escaped_quotes_in_value() {
        assert_eq!(
            ...
            Some(r#"FOO="he said \"hello\"" stc git status"#.into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_pipe() {
        assert_eq!(
            ...
            Some("stc cargo test 2>&1 | head".into())
        );
    }
    ...
    fn test_rewrite_redirect_plain_2_devnull() {
        ...
        assert_eq!(
            ...
            Some("stc git status 2>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_and() {
        assert_eq!(
            ...
            Some("stc cargo test 2>&1 && echo done".into())
        );
    }
    ...
    fn test_rewrite_redirect_amp_gt_devnull() {
        assert_eq!(
            ...
            Some("stc cargo test &>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_double() {
        ...
        assert_eq!(
            ...
            Some("stc git status 2>&1 >/dev/null".into())
        );
    }
    ...
    fn test_rewrite_head_numeric_flag() {
        // head -20 file → stc read file --max-lines 20 (not stc read -20 file)
        assert_eq!(
            rewrite_command_no_prefixes("head -20 src/main.rs", &[]),
            Some("stc read src/main.rs --max-lines 20".into())
        );
    }
    ...
    fn test_rewrite_head_lines_long_flag() {
        assert_eq!(
            ...
            Some("stc read src/lib.rs --max-lines 50".into())
        );
    }

    #[test]
    fn test_rewrite_head_no_flag_still_rewrites() {
        // plain `head file` → `stc read file` (no numeric flag)
        assert_eq!(
    ...
    fn test_rewrite_tail_numeric_flag() {
        assert_eq!(
            ...
            Some("stc read src/main.rs --tail-lines 20".into())
        );
    }
    ...
    fn test_rewrite_tail_n_space_flag() {
        assert_eq!(
            ...
            Some("stc read src/lib.rs --tail-lines 12".into())
        );
    }
    ...
    fn test_rewrite_tail_lines_long_flag() {
        assert_eq!(
            ...
            Some("stc read src/lib.rs --tail-lines 7".into())
        );
    }
    ...
    //
    // `stc read <file> --max-lines N` only accepts a single positional file path in
    // a shape that maps cleanly to `head -N`. Rewriting `head -N a b c` to
    // `stc read a b c --max-lines N` previously produced a command where `stc read`
    ...
    fn test_rewrite_diff() {
        assert_eq!(
            ...
            Some("stc diff file1.txt file2.txt".into())
        );
    }
    ...
    fn test_rewrite_cargo_install() {
        assert_eq!(
            ...
            Some("stc cargo install rtk".into())
        );
    }
    ...
    fn test_rewrite_kubectl_describe() {
        assert_eq!(
            ...
            Some("stc kubectl describe pod mypod".into())
        );
    }
    ...
    fn test_rewrite_docker_run() {
        assert_eq!(
            ...
            Some("stc docker run --rm ubuntu bash".into())
        );
    }
    ...
    fn test_rewrite_swift_test() {
        assert_eq!(
            ...
            Some("stc swift test --parallel".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_ps() {
        assert_eq!(
            ...
            Some("stc docker compose ps".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_logs() {
        assert_eq!(
            ...
            Some("stc docker compose logs web".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_build() {
        assert_eq!(
            ...
            Some("stc docker compose build".into())
        );
    }
    ...
    fn test_rewrite_aws_ec2() {
        assert_eq!(
            ...
            Some("stc aws ec2 describe-instances --region us-east-1".into())
        );
    }
    ...
    fn test_rewrite_psql() {
        assert_eq!(
            ...
            Some("stc psql -U postgres -d mydb".into())
        );
    }
    ...
    fn test_classify_golangci_lint() {
        assert!(matches!(
            ...
            Classification::Supported {
                rtk_equivalent: "stc golangci-lint run",
                ..
            }
        ));
    }
    ...
    fn test_rewrite_golangci_lint() {
        assert_eq!(
            ...
            Some("stc golangci-lint run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_flag_before_run() {
        assert_eq!(
            ...
            Some("stc golangci-lint -v run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_value_flag_before_run() {
        assert_eq!(
            ...
            Some("stc golangci-lint --color never run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_inline_config_flag_before_run() {
        assert_eq!(
            ...
            Some("stc golangci-lint --config=foo.yml run ./...".into())
        );
    }
    ...
    fn test_rewrite_env_prefixed_golangci_lint_with_value_flag_before_run() {
        assert_eq!(
            ...
            Some("FOO=1 stc golangci-lint --color never run ./...".into())
        );
    }
    ...
    fn test_rewrite_prisma() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("stc prisma migrate dev".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_prettier() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("stc prettier --check src/".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_npm_with_args() {
        assert_eq!(
            ...
            Some("stc npm run test".to_string()),
        );
        assert_eq!(
            rewrite_command_no_prefixes("npm exec vitest", &[]),
            Some("stc vitest".to_string()),
        );
    }
    ...
    fn test_rewrite_gradlew() {
        assert_eq!(
            ...
            Some("stc gradlew assembleDebug".into())
        );
    }
    ...
    fn test_rewrite_compound_or() {
        ...
        assert_eq!(
            ...
            Some("stc cargo test || stc cargo build".into())
        );
    }
    ...
    fn test_rewrite_compound_semicolon() {
        assert_eq!(
            ...
            Some("stc git status; stc cargo test".into())
        );
    }
    ...
    fn test_rewrite_compound_pipe_raw_filter() {
        ...
        assert_eq!(
            ...
            Some("stc cargo test | grep FAILED".into())
        );
    }
    ...
    fn test_rewrite_compound_four_segments() {
        assert_eq!(
            ...
            Some(
                "stc cargo fmt --all && stc cargo clippy && stc cargo test && stc git status"
                    .into()
            )
        );
    }
    ...
    fn test_rewrite_env_var_prefix() {
        assert_eq!(
            ...
            Some("GIT_SSH_COMMAND=ssh stc git push origin main".into())
        );
    }
    ...
    fn test_rewrite_find_with_flags() {
        assert_eq!(
            ...
            Some("stc find . -name '*.rs' -type f".into())
        );
    }
    ...
    fn test_rewrite_compound_partial_exclude() {
        ...
        assert_eq!(
            ...
            Some("stc git status && curl https://api.example.com".into())
        );
    }
    ...
    fn test_cmd_has_rtk_disabled_prefix() {
        ...
        assert!(!cmd_has_rtk_disabled_prefix("stc git status"));
    ...
    fn test_rewrite_git_dash_c() {
        assert_eq!(
            ...
            Some("stc git -C /tmp status".to_string())
        );
    }
    ...
    fn test_rewrite_git_no_pager() {
        assert_eq!(
            ...
            Some("stc git --no-pager log -5".to_string())
        );
    }
    ...
    fn test_rewrite_wc() {
        assert_eq!(
            ...
            Some("stc wc -l src/main.rs".into())
        );
    }
    ...
    fn test_rewrite_command_substitution_passthrough() {
        assert_eq!(
            ...
            Some("stc git log $(git rev-parse HEAD~1)".into())
        );
    }
    ...
    fn test_shell_prefix_noglob() {
        assert_eq!(
            ...
            Some("noglob stc git status".into())
        );
    }
    ...
    fn test_shell_prefix_command() {
        assert_eq!(
            ...
            Some("command stc git status".into())
        );
    }
    ...
    fn test_shell_prefix_builtin_exec_nocorrect() {
        assert_eq!(
            ...
            Some("builtin stc git status".into())
        );
        assert_eq!(
            rewrite_command_no_prefixes("exec git status", &[]),
            Some("exec stc git status".into())
        );
        assert_eq!(
            rewrite_command_no_prefixes("nocorrect git status", &[]),
            Some("nocorrect stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_strips_and_reprepends() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_with_test_runner() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- stc cargo test".into())
        );
    }
    ...
    fn test_transparent_prefix_composed_with_builtin() {
        ...
        assert_eq!(
            ...
            Some("noglob shadowenv exec -- stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_composed_with_env_prefix() {
        ...
        assert_eq!(
            ...
            Some("RAILS_ENV=test bundle exec stc git status".into())
        );
    }
    ...
    fn test_env_prefix_composed_with_builtin() {
        assert_eq!(
            ...
            Some("sudo noglob stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_multiple_configured() {
        ...
        assert_eq!(
            ...
            Some("direnv exec . stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_overlapping_entries_use_longest_match() {
        ...
        assert_eq!(
            ...
            Some("docker exec app stc git status".into())
        );
    }
    ...
    fn test_transparent_prefix_inside_compound() {
        ...
        assert_eq!(
            ...
            Some("shadowenv exec -- stc git status && shadowenv exec -- stc cargo test".into())
        );
    }
    ...
    fn test_rewrite_pipe_then_and() {
        assert_eq!(
            ...
            Some("stc git log | head -5 && stc git stash".into())
        );
    }
    ...
    fn test_rewrite_pipe_then_semicolon() {
        assert_eq!(
            ...
            Some("stc cargo test | head; stc git status".into())
        );
    }
    ...
    fn test_rewrite_pipe_then_or() {
        assert_eq!(
            ...
            Some("stc cargo test | grep FAIL || stc git stash".into())
        );
    }
    ...
    fn test_rewrite_env_pipe_then_and() {
        assert_eq!(
            ...
            Some("RUST_BACKTRACE=1 stc cargo test 2>&1 | grep FAILED && stc git stash".into())
        );
    }
    ...
    fn test_rewrite_and_then_pipe() {
        assert_eq!(
            ...
            Some("stc git status && stc cargo test | grep FAIL".into())
        );
    }
    ...
    fn test_rewrite_multi_pipe_then_and() {
        assert_eq!(
            ...
            Some("stc git log | head | tail && stc git status".into())
        );
    }
