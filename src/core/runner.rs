...
pub struct RunOptions<'a> {
    ...
    /// can read from a pipe (e.g. `cat file | stc wc`); without it the child
    ...
    match mode {
        RunMode::Filtered(filter_fn) => {
            ...
            if opts.skip_filter_on_failure && exit_code != 0 {
                ...
                timer.track(&cmd_label, &format!("stc {}", cmd_label), raw, raw);
                return Ok(exit_code);
            }
        ...
        RunMode::Passthrough => {
            ...
            timer.track_passthrough(&cmd_label, &format!("stc {} (passthrough)", cmd_label));
            Ok(result.exit_code)
        }
    }
}
