...
pub fn run(
    ...
    if summary.total_commands == 0 {
        ...
        println!("Run some stc commands to start tracking savings.");
        return Ok(());
    }
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        match hook_check::status() {
            hook_check::HookStatus::Missing => {
                eprintln!(
                    "{}",
                    "[warn] No hook installed — run `stc init -g` for automatic token savings"
                        .yellow()
                );
                eprintln!();
            }
            hook_check::HookStatus::Outdated => {
                eprintln!(
                    "{}",
                    "[warn] Hook outdated — run `stc init -g` to update".yellow()
                );
                eprintln!();
            }
...
fn check_rtk_disabled_bypass() -> Option<String> {
    ...
    if pct > 10.0 {
        Some(format!(
            "[warn] {} commands ({:.0}%) used RTK_DISABLED=1 unnecessarily — run `stc discover` for details",
            bypassed, pct
        ))
    } else {
        None
