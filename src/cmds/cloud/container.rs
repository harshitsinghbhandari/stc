...
fn docker_ps(_verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        ...
        timer.track("docker ps", "stc docker ps", &raw, &raw);
    ...
    if stdout.trim().is_empty() {
        ...
        println!("{}", rtk);
        timer.track("docker ps", "stc docker ps", &raw, &rtk);
        return Ok(0);
    }
...
fn docker_ps_all(_verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        eprint!("{}", result.stderr);
        timer.track("docker ps -a", "stc docker ps -a", &raw, &raw);
        return Ok(result.exit_code);
    }
    ...
    print!("{}", rtk);
    timer.track("docker ps -a", "stc docker ps -a", &raw, &rtk);
    Ok(0)
}
...
fn docker_images(_verbose: u8) -> Result<i32> {
    ...
    if !result.success() {
        eprint!("{}", result.stderr);
        timer.track("docker images", "stc docker images", &raw, &raw);
        return Ok(result.exit_code);
    }
    ...
    if lines.is_empty() {
        ...
        println!("{}", rtk);
        timer.track("docker images", "stc docker images", &raw, &rtk);
        return Ok(0);
    }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<i32> {
    ...
    if container.is_empty() {
        println!("Usage: stc docker logs <container>");
        return Ok(0);
    }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<i32> {
    ...
    if pod.is_empty() {
        println!("Usage: stc kubectl logs <pod>");
        return Ok(0);
    }
...
pub fn run_compose_ps(all: bool, verbose: u8) -> Result<i32> {
    ...
    let rtk_label = if all { "stc docker compose ps -a" } else { "stc docker compose ps" };
