...
fn parse_find_args(args: &[String]) -> Result<FindArgs> {
    ...
    if has_unsupported_find_flags(args) {
        anyhow::bail!(
            "stc find does not support compound predicates or actions (e.g. -not, -exec). Use `find` directly."
        );
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            flag if flag.starts_with('-') => {
                eprintln!("stc find: unknown flag '{}', ignored", flag);
            }
            _ => {}
        }
        i += 1;
