...
fn run_list(depth: usize, args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track(
        ...
        &format!("stc pnpm list --depth={}", depth),
        &result.stdout,
        &filtered,
    );
...
fn run_outdated(args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track("pnpm outdated", "stc pnpm outdated", &combined, &filtered);
...
fn run_install(args: &[String], verbose: u8) -> Result<i32> {
    ...
    timer.track("pnpm install", "stc pnpm install", &combined, &filtered);
