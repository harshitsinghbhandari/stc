...
Owns: `stc init` installation flows (5 agents via `AgentTarget` enum + 3 special modes: Gemini, Codex, OpenCode — each with its own programmatic hook protocol and config file), SHA-256 integrity verification, hook version checking, audit log analysis, `stc rewrite` CLI entry point, and TOML filter trust management.
...
Boundary notes:
- `rewrite_cmd.rs` is a thin CLI bridge — it exists to serve hooks (hooks call `stc rewrite` as a subprocess) and delegates entirely to `discover/registry`.
...
## Purpose
LLM agent integration layer that installs, validates, and executes command-rewriting hooks for AI coding assistants. Hooks intercept raw CLI commands (e.g., `git status`) and rewrite them to RTK equivalents (e.g., `stc git status`) so that LLM agents automatically benefit from token savings without explicit user configuration.

## Installation Modes

`stc init` supports these installation flows:

| Mode | Command | Creates | Patches |
|------|---------|---------|----------|
| Default (global) | `stc init -g` | Hook, SHA-256 hash, RTK.md | settings.json, CLAUDE.md |
| Hook only | `stc init -g --hook-only` | Hook, SHA-256 hash | settings.json |
| Claude-MD (legacy) | `stc init --claude-md` | 134-line RTK block | CLAUDE.md |
| Windsurf | `stc init -g --agent windsurf` | `.windsurfrules` | -- |
| Cline | `stc init --agent cline` | `.clinerules` | -- |
| Codex | `stc init --codex` | RTK.md in `$CODEX_HOME` or `~/.codex` | AGENTS.md, hooks.json (`PreToolUse` → `stc hook codex`) |
| Cursor | `stc init -g --agent cursor` | Cursor hook | hooks.json |
| Pi | `stc init --agent pi` | `.pi/extensions/rtk.ts` | -- |
| Hermes | `stc init --agent hermes` | Python plugin in `~/.hermes/plugins/rtk-rewrite/` | `config.yaml` `plugins.enabled` |
...
3. On demand: `stc verify` prints detailed verification status (PASS/FAIL/WARN/SKIP)
...
Controls how `stc init` modifies agent settings files:
...
All file operations use atomic writes (tempfile + rename) to prevent corruption on crash. Settings files are backed up to `.bak` before modification. All operations are idempotent -- running `stc init` multiple times is safe.
...
- `hook_cmd.rs` — maps verdict to JSON `permissionDecision` field (Claude/Copilot/Gemini/Cursor/Codex)
...
To add support for a new AI coding agent: (1) add the hook installation logic to `init.rs` following the existing agent patterns, (2) if the agent requires a custom hook protocol (like Gemini's `BeforeTool`), add a processor function in `hook_cmd.rs`, (3) add the agent's hook file path to `hook_check.rs` for validation, and (4) update `integrity.rs` with the expected hash for the new hook file. Test by running `stc init` in a fresh environment and verifying the hook rewrites commands correctly in the target agent.
